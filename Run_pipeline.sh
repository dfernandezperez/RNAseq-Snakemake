# Usage --> ./Run_pipeline.sh  <rules> (can be nothing or a rule defined in the snakefile without wildcards in the input files)     (the samples.json file must be created first, also remember to check the cluster and config files to put everything to your needs
snakemake --unlock
